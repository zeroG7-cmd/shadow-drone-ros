...
class MissionPlanner(Node):
    ...
    def _publish_setpoint(self, xyz):
        # PositionTarget with yaw ignored via type_mask - we ask PX4 to
        # control position only, not orientation, since heading_good_
        # for_control is false and we confirmed (via Iris too) this
        # isn't specific to Shadow - it's this whole SITL setup. No
        # point fighting PX4 for yaw control it's told us it doesn't
        # trust yet.
        IGNORE_YAW = 0b100000000000  # bit 11
        IGNORE_YAW_RATE = 0b1000000000000  # bit 12

        target = PositionTarget()
        target.header.stamp = self.get_clock().now().to_msg()
        target.header.frame_id = 'map'
        target.coordinate_frame = PositionTarget.FRAME_LOCAL_NED
        target.type_mask = IGNORE_YAW | IGNORE_YAW_RATE
